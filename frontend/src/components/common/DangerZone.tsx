...
export function DangerZone({
  ...
  return (
    <Card className={`rounded-lg border shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden 
      ${isCritical 
        ? "bg-red-50/50 border-red-900/30" 
        : "border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20"}`}
    >
      <CardHeader className={isCritical 
      ...
      <CardContent className={isCritical ? "p-6" : ""}>
        <div className={`flex ${isCritical ? "flex-col lg:flex-row items-start lg:items-center" : "items-center"} justify-between gap-6 
        ...
        >
          <div className="flex-1">
            ...
            <p className={`${isCritical ? "text-sm text-red-700/80 leading-relaxed mb-3" : "text-sm text-red-600"} 
              ${isCritical ? "pl-10" : ""}`}
            >
              {description}
            </p>
