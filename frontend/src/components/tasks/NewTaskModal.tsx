    ...
    try {
      ...
      if (urlContext.type === "global") {
        ...
        await loadProjectFromUrl(
          urlContext.workspaceSlug,
          urlContext.projectSlug
        );
      }
    } catch (error) {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (!defaultStatus) {
          throw new Error(
            "No task status available. Please contact your administrator."
          );
        }
        ...
        if (onTaskCreated) {
          try {
            ...
            toast.warning(
              "Task created but failed to refresh list. Please refresh the page."
            );
          }
        }
              ...
              >
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className="projects-workspace-button border-none"
                    disabled={
                      !formData.workspace || loadingProjects || isSubmitting
                    }
                    onFocus={(e) => {
                      e.currentTarget.style.boxShadow = "none";
                    }}
