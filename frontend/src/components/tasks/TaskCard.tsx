...
export default function TaskCard({ task, allTasks = [], onDragStart, onDragEnd, isDragging }: TaskCardProps) {
  ...
  const formatDueDate = (dateString: string) => {
    ...
    if (diffDays < 0) {
      return { text: `${Math.abs(diffDays)} days overdue`, color: 'text-red-600', bgColor: 'bg-red-100 dark:bg-red-900/20' };
    } else if (diffDays === 0) {
      return { text: 'Due today', color: 'text-orange-600', bgColor: 'bg-orange-100 dark:bg-orange-900/20' };
    } else if (diffDays === 1) {
      return { text: 'Due tomorrow', color: 'text-yellow-600', bgColor: 'bg-yellow-100 dark:bg-yellow-900/20' };
    } else if (diffDays <= 7) {
      return { text: `Due in ${diffDays} days`, color: 'text-yellow-600', bgColor: 'bg-yellow-100 dark:bg-yellow-900/20' };
    } else {
      return { text: date.toLocaleDateString(), color: 'text-gray-600', bgColor: 'bg-gray-100 dark:bg-gray-900/20' };
    }
  };
        ...
        {task.dueDate && (
          <div className="mb-3">
            {(() => {
              ...
              return (
                <span className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full ${dueDateInfo.bgColor} ${dueDateInfo.color}`}>
      ...
      {showDetailModal && (
        <TaskDetailModal
          ...
          onUpdate={(updatedTask) => {
            // In real app, update the task in the list
