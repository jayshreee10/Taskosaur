...
export class LocalCommandParser {
  ...
  parseCommand(input: string): ParsedCommand | null {
    ...
    if (lower.includes('list') && lower.includes('workspace') ||
    ...
    // Filter by priority
    if (lower.match(/(show|filter|display|get)\s*(all)?\s*(high|medium|low|highest)\s*priority\s*tasks?/)) {
    ...
    // Search tasks
    const searchMatch = input.match(/^(search|find|look)\s+(?:for\s+)?(?:tasks?\s+)?["\']?([^"']+)["\']?$/i);
    if (searchMatch) {
      return {
        action: 'searchTasks',
    ...
    const navWorkspaceMatch = input.match(/^(go|navigate|switch|redirect)\s+to\s+([^\s]+)\s+workspace$/i);
    ...
    if (lower.includes('redirect') && lower.includes('workspace') ||
