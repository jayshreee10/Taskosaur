...
export const taskApi = {
 ...
 getTasksByOrganization: async (organizationId: string): Promise<Task[]> => {
    try {
      ...
      const response = await api.get<Task[]>(`/tasks/organization/${organizationId}`);
      return response.data;
    } catch (error) {
  ...
  getSubtasksByParent: async (parentTaskId: string): Promise<Task[]> => {
    try {
      const response = await api.get<Task[]>(`/tasks?parentTaskId=${parentTaskId}`);
      return response.data;
    } catch (error) {
  ...
  getTaskComments: async (taskId: string): Promise<TaskComment[]> => {
    try {
      const response = await api.get<TaskComment[]>(`/task-comments?taskId=${taskId}`);
      return response.data;
    } catch (error) {
  ...
  updateTaskComment: async (commentId: string, userId: string, commentData: UpdateTaskCommentRequest): Promise<TaskComment> => {
    try {
      const response = await api.patch<TaskComment>(`/task-comments/${commentId}?userId=${userId}`, commentData);
      return response.data;
    } catch (error) {
  ...
  uploadAttachment: async (taskId: string, file: File): Promise<TaskAttachment> => {
    try {
      ...
      const response = await api.post<TaskAttachment>(`/task-attachments/upload/${taskId}`, formData, {
        headers: {
  ...
  getTaskAttachments: async (taskId: string): Promise<TaskAttachment[]> => {
    try {
      ...
      const response = await api.get<TaskAttachment[]>(`/task-attachments/task/${taskId}`);
      return response.data;
    } catch (error) {
  ...
  deleteAttachment: async (attachmentId: string, requestUserId: string): Promise<void> => {
    try {
      console.log('Deleting attachment:', attachmentId, 'by user:', requestUserId);
  ...
  removeLabelFromTask: async (taskId: string, labelId: string): Promise<void> => {
    try {
      ...
      await api.delete(`/task-labels/remove?taskId=${taskId}&labelId=${labelId}`);
  ...
  searchLabels: async (projectId: string, query: string): Promise<TaskLabel[]> => {
    try {
      console.log('Searching labels for project:', projectId, 'with query:', query);
      const response = await api.get<TaskLabel[]>(`/labels/search?projectId=${projectId}&q=${encodeURIComponent(query)}`);
      return response.data;
    } catch (error) {
