  ...
  private getCommentsDataForTask(task: Task) {
    ...
    if (task.type === 'BUG') {
      return [
        ...
        {
          content: 'Looking into this now. It seems to be related to the recent changes in the authentication flow.',
        },
        {
          content: 'Fixed the issue by updating the validation logic. The problem was with how we handle edge cases in user input.',
        },
      ];
    } else if (task.type === 'STORY') {
      return [
        {
          content: 'This feature will significantly improve user experience. I suggest we also consider mobile responsiveness from the start.',
        },
        {
          content: 'Great idea! I\'ve created some initial mockups. Should we schedule a design review meeting?',
        },
        {
          content: 'The implementation is progressing well. I\'ve completed about 70% of the functionality.',
        },
      ];
    } else if (task.type === 'EPIC') {
      return [
        {
          content: 'This is a major feature that will span multiple sprints. Let\'s break it down into smaller, manageable tasks.',
        },
        {
          content: 'I\'ve outlined the technical approach in the attached document. We should discuss the architecture before proceeding.',
        },
      ];
    } else {
