...
export class UniversalSearchService {
    ...
    private async validateOrganizationAccess(organizationId: string, userId: string): Promise<{ isElevated: boolean }> {
        try {
    ...
    private scoreResults(results: SearchResult[], query: string): SearchResult[] {
        ...
        return results.map(result => {
            ...
            const daysSinceUpdate = (Date.now() - result.metadata.updatedAt.getTime()) / (1000 * 60 * 60 * 24);
