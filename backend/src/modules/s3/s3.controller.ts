...
export class S3Controller {
  ...
  @ApiQuery({ name: 'key', description: 'S3 object key (path/filename)', required: true })
  @ApiResponse({ status: 200, description: 'Presigned PUT URL', schema: { example: { url: 'https://...' } } })
  ...
  @ApiOperation({ summary: 'Get presigned GET URL to download or access a file' })
  @ApiQuery({ name: 'key', description: 'S3 object key (path/filename)', required: true })
  @ApiResponse({ status: 200, description: 'Presigned GET URL', schema: { example: { url: 'https://...' } } })
